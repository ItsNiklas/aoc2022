...
def get_solution_paths_dict_for_years() -> dict[int, dict[int, list[str]]]:
    ...
    for day_file in sorted(Path(year_dir).glob('*.py')):

        day = find_first_number(day_file.name)
        if day == 0: continue

        print(day_file.absolute())
        ...
        solution = day_file.relative_to(AOC_DIR)

        solution_paths_dict[year][day] = [str(solution)]
